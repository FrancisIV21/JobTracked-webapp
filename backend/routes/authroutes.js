...
router.get('/google/callback', (req, res, next) => {
  ...
  passport.authenticate('google', {
    ...
    if (err) {
      ...
      return res.redirect(`${process.env.FRONTEND_URL}/JobTrackerSignUp?error=auth_failed&details=${encodeURIComponent(err.message)}`);
    }

    if (!user) {
      console.error('No user returned from Google OAuth');
      console.error('Passport info:', info);
      return res.redirect(`${process.env.FRONTEND_URL}/JobTrackerSignUp?error=no_user`);
    }
    ...
    try {
      ...
      req.logIn(user, (loginErr) => {
        if (loginErr) {
          ...
          return res.redirect(`${process.env.FRONTEND_URL}/JobTrackerSignUp?error=session_error&details=${encodeURIComponent(loginErr.message)}`);
        }
        ...
        // Get return URL
        const returnTo = req.session.returnTo || '/JobTrackerDashboard';
      ...
      res.redirect(`${process.env.FRONTEND_URL}/JobTrackerSignUp?error=${errorType}&details=${encodeURIComponent(error.message)}`);
    }
  })(req, res, next);
});
